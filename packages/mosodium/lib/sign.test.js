...
describe('sign Suite', () => {
  ...
  it('should sign and verify', () => {
    ...
    const keys = sodium.sign.seedKeypair(Buffer.alloc(32))

    console.log("PASS",keys.privateKey);
    const sig = sodium.sign.sign(Buffer.from('hello'), keys.privateKey)
    const ver = sodium.sign.verify(sig, Buffer.from('hello'), keys.publicKey)
